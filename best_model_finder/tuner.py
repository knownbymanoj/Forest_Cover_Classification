...
class Model_Finder:
    ...
    def \
            get_best_params_for_xgboost(self,train_x,train_y):
    ...
    def  get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            self.prediction_proba_xgboost = self.xgboost.predit_proba(test_x)
            ...
            if len(test_y.unique()) == 1: #if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                ...
                self.xgboost_prediction_random_forestscore = roc_auc_score(test_y, self.prediction_proba_xgboost, multi_class='ovr') # AUC for XGBoost
            ...
            self.prediction_proba_random_forest = self.random_forest.predict_proba(test_x)

            if len(test_y.unique()) == 1:#if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                self.random_forest_score = accuracy_score(test_y,self.prediction_random_forest)
                self.logger_object.log(self.file_object, 'Accuracy for RF:' + str(self.random_forest_score))
            else:
                self.random_forest_score = roc_auc_score(test_y, self.prediction_proba_random_forest,multi_class='ovr') # AUC for Random Forest
